...
action(user.code_state_when_condition):
  insert("\n -> ")
  key(left left left left)
...
action(user.code_try_catch):
  insert("try {}")
  key(left enter down end)
  insert(" catch (e: Exception) {}")
  key(left enter up up end)
...
active language: "Current language: Kotlin"
...
^protected function <user.text>$: user.code_protected_function(text)
^public function <user.text>$: user.code_public_function(text)